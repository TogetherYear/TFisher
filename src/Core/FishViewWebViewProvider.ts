...
class FishViewWebViewProvider {
    private constructor(context: vscode.ExtensionContext, viewColumn: vscode.ViewColumn, label: string, icon: string) {
    ...
    private CreateView() {
        ...
        webViewPanel.iconPath = vscode.Uri.file(path.join(this.context.extensionPath, `/resources/${this.icon}.png`))
    ...
    private RefreshNew() {
        return new Promise((resolve, reject) => {
            A.get('https://www.zhihu.com/api/v3/feed/topstory/recommend?action=down&ad_interval=-10&desktop=true').then(res => {
                const target = res.data.data
                const result = []
                for (let i of target) {
                    result.push({ question: i.target.question.title, answer: i.target.content.replace(/<\/?.+?\/?>/g, "") })
                }
                resolve(result)
            }).catch((err) => {
                reject("Error")
    ...
    public static Generate(context: vscode.ExtensionContext, viewColumn: vscode.ViewColumn, label: string, icon: string) {
