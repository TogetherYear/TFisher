...
const CreateV3Component = (uri: vscode.Uri) => {
    ...
    vscode.window.showInputBox({ placeHolder: '请输入组件名称' }).then((name) => {
        if (name) {
            ...
            const vue = fs
                .readFileSync(path.join(__dirname, '../resources/Template/Template.vue.t'), 'utf-8')
                .replaceAll('$0', name);
            const ts = fs
                .readFileSync(path.join(__dirname, '../resources/Template/Template.ts.t'), 'utf-8')
                .replaceAll('$0', name);
            const scss = fs
                .readFileSync(
                    path.join(__dirname, '../resources/Template/Template.scss.t'),
                    'utf-8'
                )
                .replaceAll('$0', name);
